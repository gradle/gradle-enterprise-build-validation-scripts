...
source "${LIB_DIR}/02/parsing.sh" || { echo "Couldn't find '${LIB_DIR}/02/parsing.sh' parsing library."; exit 1; }
...
execute_build() {
  ...
  invoke_gradle \
     --init-script "${lib_dir_rel}/verify-and-configure-local-build-cache-only.gradle" \
     clean "${task}"
}
