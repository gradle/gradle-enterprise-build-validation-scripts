...
final class GradleScriptsTest extends BaseScriptsTest {
    ...
    private String develocityServer
    ...
    def "can set Develocity server using --develocity-server"() {
        given:
        gradleVersion = version
        develocityServer = "https://develocity-server.invalid"

        when:
        run experiment, "--develocity-server", develocityTestUrl
    ...
    def "can inject Develocity plugin using --enable-develocity"() {
        ...
        when:
        run experiment, "--enable-develocity", "--develocity-server", develocityTestUrl
    ...
    def "experiments and injection are compatible with Gradle configuration cache"() {
        ...
        when:
        run experiment, "--enable-develocity", "--develocity-server", develocityTestUrl, "--args", "--configuration-cache"
    ...
    void buildTestProject() {
        def develocityServer = develocityServer ?: develocityTestUrl
