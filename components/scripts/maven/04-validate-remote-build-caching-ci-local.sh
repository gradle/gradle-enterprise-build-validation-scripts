...
validate_required_args() {
  ...
  if [[ "${enable_ge}" == "on" && -z "${ge_server}" ]]; then
    _PRINT_HELP=yes die "ERROR: Missing required argument when enabling Develocity on a project not already connected: --gradle-enterprise-server" "${INVALID_INPUT}"
  fi
}
...
The build scan data will be fetched via the Develocity Export API. It is not
strictly necessary that you have permission to call the Export API while doing
this experiment, but the summary provided at the end of the experiment will be
more comprehensive if the build scan data is accessible. You can check your
granted permissions by navigating in the browser to the 'My Settings' section
from the user menu of your Develocity UI. Your Develocity access key must be
specified in the ~/.m2/.gradle-enterprise/keys.properties file.
