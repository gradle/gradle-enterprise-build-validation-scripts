...
print_maven_command() {
  if [[ "${build_scan_publishing_mode}" == "on" ]]; then
    ...
    info "./mvnw -Dscan.dump -Dscan.tag.${EXP_SCAN_TAG} -Dscan.value.runId=${RUN_ID} -Dpts.enabled=false clean ${tasks}$(print_extra_args)"
  fi
}
...
well your build leverages Gradle Enterprise's local build caching functionality
for the invoked set of Maven goals.
